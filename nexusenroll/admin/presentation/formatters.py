"""
nexusenroll/admin/presentation/formatters.py

Role: PRESENTATION TIER — plain-text table rendering.

No external dependencies — builds aligned ASCII tables using only the
standard library. Works generically off lists of dicts (column name ->
value), which is exactly what ReportingService/Report.to_rows() and
the entity-listing helpers in cli.py produce, so the same function
renders reports AND plain entity listings (course lists, student
lists, etc.) without duplicated formatting code (DRY).
"""
