"""
nexusenroll/admin/service/course_service.py
...
Both depend only on repository abstractions
(nexusenroll/admin/data/repositories.py), not concrete CSV-backed
classes — Dependency Inversion Principle.
"""

from typing import Optional

from nexusenroll.admin.data.entities import Course, Program, Schedule
from nexusenroll.admin.data.repositories import CourseRepository, ProgramRepository
...
class CourseService:
    ...
    def edit_course(self, course_id: str, **updates) -> Course:
        ...
        for field_name in ("name", "description", "department", "instructor_id"):
            if field_name in updates:
                setattr(course, field_name, updates[field_name])
        if "schedule" in updates:
            new_schedule = updates["schedule"]
            # The CLI only collects one line of free text for schedule
            # edits, so a bare string is wrapped the same way
            # CourseFactory wraps it for course creation (see
            # patterns/factories.py) — callers passing an already-built
            # Schedule object pass straight through unchanged.
            course.schedule = Schedule.freetext(new_schedule) if isinstance(new_schedule, str) else new_schedule
